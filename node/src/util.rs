    ...
    for node in nodes {
        ...
        match message {
            Message::NodeList(child_nodes) => {
                println!("recevied NodeList from {}", node);
...
pub async fn find_longest_chain_node() -> Result<(String, u32)> {
    ...
    for node in all_nodes {
        ...
        println!("snet AskDifference to {}", node);
        let message = Message::receive_async(&mut *stream).await?;
        match message {
            Message::Difference(count) => {
                println!("recevied Difference from {}", node);
                if count > longest_count {
                    println!("new longest blockchain: {count} blocks from {node}");
                    longest_count = count;
                    longest_name = node;
                }
            }
            e => {
                println!("unexpected message form {}: {:?}", node, e);
            }
        }
    }
